...
int main() {
    ...
    if (load_libraries("libs") != 0) {
        fprintf(stderr, "Ошибка при загрузке библиотек\n");
        return 1;
    }
    
    printf("Доступно %d бинарных и %d унарных операций\n\n", binary_operations_count, unary_operations_count);
